...
def cal_rougeL(reference, hypothesis):
    '''
    : Precision =  LCS length / Length of Hypothesis
    : Recall    =  LCS length / Length of Reference
    
    fmeasure of ROUGE-L is for article matching 
    '''
...
def cal_split_rougeL(reference, hypothesis):
    ...
    fmeasure of ROUGE-L is for article matching 
    '''
    split_ref = split_into_sentences(reference)
    
    average_scores = 0
    
    for ref in split_ref:    
        scorer = rouge_scorer.RougeScorer(['rougeL'], use_stemmer=True)
        scores = scorer.score(reference, ref)
        average_scores += scores['rougeL']
    return average_scores / len(split_ref)
    ...
    # for iter in range(0, 1):
        ...
        for k in evaluation.keys():
            ...
            if k in ["clean_retained_score", "mistake_presence_score", "hint_presence_score"]:
                ...
                continue
            if k == "unrelated_num":
                printText += (f"{k}: {evaluation[k]} / {len(split_into_sentences(response))}") + "\n"
